...
export function DetailedStats({ stats, language }: DetailedStatsProps) {
  const winRate = statsManager.calculateWinRate();
  const averageGuesses = statsManager.calculateAverageGuesses();

  // Create distribution object from guessDistribution
  const distribution: GuessDistribution = {
    total: stats.gamesWon,
    ...stats.guessDistribution
  };

  // Ensure all guess counts (1-6) exist in distribution
  for (let i = 1; i <= 6; i++) {
